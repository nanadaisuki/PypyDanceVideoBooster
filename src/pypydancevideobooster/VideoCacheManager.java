...
import static pypydancevideobooster.PypyDanceVideoBooster.headersToString;
...
public class VideoCacheManager {
    ...
    public static void download(String videoId, CDN cdn, DownloadLimiter... limiter) {
        if (limiter.length > 0) {
            System.out.println("Download rate limit " + limiter[0].getMbRatePerSec() + "MB/s");
        }
        ...
        try {
            ...
            while (readCount < length) {
                ...
                if (limiter.length > 0) {
                    limiter[0].limitRate(lastRemain - remain);
                }
                lastRemain = remain;
